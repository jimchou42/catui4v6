...
(function ($) {
                ...
                if (pos >= 0) {
                    if (transport) {
                        if (metas[colName].type === "N") {
                            ...
                            row[pos] = $.isNumeric(rowJson[colName]) ? (rowJson[colName] + "") : (rowJson[colName] || "") + "";
                        }
                    } else {
    ...
    kendo.ui.Tooltip.fn.hide = (function (hide) {
        return function (isClick) {
            if (this.options.filter === '.erpcat-tooltip') {
                if (!!isClick) {
                    hide.call(this);
                }
            } else {
    ...
    kendo.ui.Grid.prototype.removeCheckedRows = function (isValid, msg) {
        ...
        if (isValid) {
            ...
            if (length > 0) {
                PSCSUtil.confirm("确定对勾选中的[" + rows.length + "]条数据做\""
    ...
    kendo.ui.Grid.prototype.removedDataItems = function (isValid, msg) {
        ...
        if (isValid) {
            ...
            PSCSUtil.confirm("确定对[" + rows.length + "]条数据做\""
                + defaultIfEmpty(msg, "移除") + "\"操作?", {
                ok: function () {
                    for (var i = 0; i < length; i++) {
                        that.dataSource.remove(rows[i])
    ...
    kendo.ui.Grid.prototype.isSelectedRows = function (msg) {
        ...
        if (rowsdata.length > 0) {
            ...
            WindowUtil({
                'title': "操作提示",
                "content": "<div class='kendo-del-message'>" + defaultIfEmpty(msg, '请选中记录再操作') + "</div>"
            });
            return false;
        }
    ...
    PSCSColorbox = {
        ...
        open: function (parameter) {
            ...
            var divhtml = $("#PSCSColorboxWin");
            if (divhtml.length > 0) {

            } else {
                var divStr = "<div id='PSCSColorboxWin'> </div>";
                $("body").append(divStr);
                divhtml = $("#PSCSColorboxWin");
            }
            ...
            if (parameter.params) {
                parameter["href"] = this.setParams(parameter.params, parameter["href"]);
            }
            ...
            $("#PSCSColorboxWin_wnd_title").html(parameter["title"]);
                    ...
                    onSuccess: function (ei) {
                        ...
                        if (ei.getStatus() >= 0) {
                            if (isRefreshre) {
                                ...
                                if (IPLAT.isAvailable(grid_ids)) {
                                    for (var j = 0, count = _grid_ids.length; j < count; j++) {
                                        // PSCSUtil.grid_setData(ei.getBlock(_grid_ids[j]),_grid_ids[j]);
                                        window[_grid_ids[j] + 'Grid'].setEiInfo(ei);
	...
	var PsAlert = function(message, title) {
		var content = kendo.template($("#alert-template").html())({
			message : message,
			ok : '确定'
		});
		WindowUtil({
    ...
    var block2String = function (regionId) {
        ...
        // 这里根据PSCS项目组需求，把select 多选下拉框里面的值中存在',' 替换为 ';'
    ...
    var submitGrid = function (grid_ids, service_name, method_name, parameter) {
        ...
        var obj = {isGetForm: true, isRefreshre: true, isProgress: true, isAlldata: false};
    ...
    /*
     * PSCSUtil.attachManagement( { docPath:__eiInfo.get("pathValue"), docTag:
    ...
    // FIXME 2017-10-08 HK 覆盖解决0915版本EFInput format的问题
